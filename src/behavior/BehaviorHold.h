 ...
 *under the terms of the GNU General Public License      ribbl                *
...
public:
  ...
  void SubmitVisualRequest(const ActiveBehavior &hold, double plus = 0.0) {
    ...
    VisualSystem::instance().RaisePlayer(mAgent, -hold.mKeyOppGB.mUnum,
                                         2.0 + plus);
    VisualSystem::instance().RaisePlayer(mAgent, -hold.mKeyOppGT.mUnum,
                                         2.0 + plus);
  }
