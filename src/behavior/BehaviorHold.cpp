...
bool BehaviorHoldExecuter::Execute(const ActiveBehavior &hold) {
  Logger::instance().LogDribble(mBallState.GetPos(), hold.mTarget, "@Hold",
                                true);
  //	Logger::instance().GetTextLogger("Hold")<< mWorldState.CurrentTime(). +
  //"Hold";
  if (hold.mDetailType == BDT_Hold_Turn) {
    return Dasher::instance()
        .GetTurnBodyToAngleAction(mAgent, hold.mAngle)
        .Execute(mAgent);
  } else
  ...
  if (mStrategy.GetSureOppInterCycle() <= 2 &&
    ...
    if (mBallState.GetPos().Dist(posAgent + Polar2Vector(0.7, ang)) < 0.3) {
      ...
      if (posPred.Dist(posBallPred) < 0.85 * mSelfState.GetKickableArea()) {
        ...
        hold.mAngle = (Vector(ServerParam::instance().PITCH_LENGTH / 2.0, 0.0) -
                       mSelfState.GetPos())
                          .Dir();
